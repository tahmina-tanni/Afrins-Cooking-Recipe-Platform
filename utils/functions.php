...
function generateRecipeCard($recipe, $showRating = true) {
    ...
    if ($showRating) {
        ...
        $reviews = isset($recipe['reviews']) ? $recipe['reviews'] : 0;

        $ratingHtml = '
            <div class="flex items-center mt-2">
                <div class="star-rating flex">' . $stars . '</div>
                <span class="text-gray-600 text-sm ml-1">(' . $reviews . ' reviews)</span>
            </div>';
    }
...
function generatePagination($current_page, $total_pages, $base_url) {
    ...
    if ($current_page > 1) {
        ...
        $html .= '<a href="' . htmlspecialchars($prev_url) . '"
    ...
    for ($i = $start; $i <= $end; $i++) {
        if ($i == $current_page) {
            ...
            $html .= '<a href="' . htmlspecialchars($page_url) . '"
    ...
    if ($current_page < $total_pages) {
        ...
        $html .= '<a href="' . htmlspecialchars($next_url) . '"
